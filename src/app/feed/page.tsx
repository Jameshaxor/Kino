...
import { Play, TrendingUp, Calendar, Star, Clock, Megaphone, Share2 } from "lucide-react";
...
async function getFeedData() {
  ...
  try {
    ...
    // Fetch trailers for trending items
    const feedItems: any[] = [];

    // Process trailers
    for (const item of trending.slice(0, 4)) {
    ...
    // Process trending announcements
    for (const item of trending.slice(4, 9)) {
      feedItems.push({
    ...
    // Sort by date (future & newest first)
...
export default async function FeedPage() {
  ...
  const img = (path: string, size = "w780") => `https://image.tmdb.org/t/p/${size}${path}`;
  ...
  return (
    <div className="max-w-3xl mx-auto w-full px-5 md:px-0 py-12 md:py-20 flex flex-col gap-10">
      ...
      <div className="flex flex-col gap-12">
        {feedItems.map((item) => (
          <article key={item.id} className="flex flex-col gap-4 pb-12 border-b border-border/50 last:border-0 relative">
            ...
            <div className="card-premium overflow-hidden rounded-2xl group flex flex-col">
                ...
                <Link href={`/${item.content.media_type === "tv" ? "tv" : "movie"}/${item.content.id}`}>
                  <div className="relative aspect-[16/9] w-full overflow-hidden border-b border-border/50 bg-bg-elevated">
                    {item.content.backdrop_path ? (
                      <Image
                        src={img(item.content.backdrop_path)}
              ...
              <div className="p-5 flex flex-col gap-3">
                <div className="flex items-start justify-between gap-4">
                  <Link href={`/${item.content.media_type === "tv" || (!item.content.title && item.content.name) ? "tv" : "movie"}/${item.content.id}`} className="hover:text-accent transition-colors relative z-20">
                ...
                <div className="flex flex-wrap items-center gap-2 mt-2">
                  <Link 
                    ...
                    className="relative z-20 text-xs font-bold px-4 py-2 bg-accent/10 border border-accent/20 rounded-md hover:bg-accent/20 text-accent transition-all flex items-center gap-1 uppercase tracking-wider"
                  >
                    View Details
                  </Link>
                </div>
