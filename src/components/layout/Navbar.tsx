...
import { Search, Sparkles, Bookmark, Compass, Menu, X, LogOut, User as UserIcon, TrendingUp } from "lucide-react";
...
export default function Navbar({ onOpenSearch }: NavbarProps) {
  ...
  const navLinks = [
    ...
    { href: "/ai", label: "AI Oracle", icon: Sparkles },
    ...
    <>
      <header
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ease-out border-b ${
          scrolled
            ? "bg-bg-primary/90 backdrop-blur-xl border-border/50 py-3 shadow-[0_8px_30px_rgb(0,0,0,0.4)]"
          ...
          <div className="flex items-center justify-between">
            ...
            <nav className="hidden md:flex items-center gap-7">
              {navLinks.map((link) => {
                ...
                return (
                  <Link key={link.href} href={link.href} className={`relative flex items-center gap-1.5 text-sm font-medium transition-colors ${
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="flex items-center gap-2 px-3 py-2 md:px-4 md:py-2 rounded-lg bg-bg-surface border border-border text-text-secondary hover:text-text-primary hover:border-border-light transition-all"
              >
                <Search className="w-3.5 h-3.5 md:w-4 md:h-4" />
                <span className="text-xs hidden sm:inline">Search</span>
                <kbd className="hidden sm:inline text-[10px] px-1.5 py-0.5 rounded bg-bg-elevated border border-border font-mono text-text-tertiary">⌘K</kbd>
              </button>
              ...
              {user ? (
                ...
                <button
                  ...
                  className="btn-gold px-3 py-1.5 md:px-4 md:py-2 text-xs md:text-sm flex items-center gap-1.5"
                >
